...
async function traerPedidos() {
  ...
  for (let page = 1; page <= 5; page++) {
    const url = `${base}?status=open&payment_status=paid&shipping_status=unpacked&per_page=50&page=${page}`;
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // GET => devolver pedidos (la app lo llama con /api/tiendanube?empresa=...)
    if (req.method === "GET") {
